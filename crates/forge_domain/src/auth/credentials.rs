...
impl AuthDetails {
    ...
    /// Returns true if these auth details are ephemeral and should not be
    /// persisted to disk.  Currently only `HelperCommand`-backed API keys are
    /// ephemeral — they are re-created from environment variables on each
    /// startup.
    pub fn is_ephemeral(&self) -> bool {
        matches!(self, Self::ApiKey(ApiKeyProvider::HelperCommand { .. }))
    }
