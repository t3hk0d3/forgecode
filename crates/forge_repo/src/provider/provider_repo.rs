...
struct ProviderConfig {
    ...
    #[serde(default)]
    #[merge(strategy = overwrite)]
    api_key_helper_var: Option<String>,
    #[serde(default)]
...
impl From<forge_config::ProviderEntry> for ProviderConfig {
    fn from(entry: forge_config::ProviderEntry) -> Self {
        ...
        ProviderConfig {
            ...
            api_key_helper_var: entry.api_key_helper_var,
...
> ForgeProviderRepository<F>
{
    ...
    async fn create_credential_from_env(
        ...
        // Check for helper command env var (explicit or fallback convention)
        let helper_var = config
            .api_key_helper_var
            .clone()
            .or_else(|| config.api_key_vars.as_ref().map(|v| format!("{v}_HELPER")));

        // TODO: remove debug logging
        if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true).open("/tmp/forge_helper_debug.log") {
            use std::io::Write;
            let _ = writeln!(f, "provider={} helper_var={:?} env_value={:?}",
                config.id,
                helper_var,
                helper_var.as_ref().and_then(|v| self.infra.get_env_var(v)),
            );
        }

        if let Some(ref helper_var_name) = helper_var {
            if let Some(command) = self.infra.get_env_var(helper_var_name) {
